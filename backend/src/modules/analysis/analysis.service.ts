...
export class AnalysisService {
  ...
  async deleteAnalysis(analysisId: string, userId: string, role: string) {
    const analysis = await this.prisma.analysis.findUnique({
      where: { id: analysisId },
      include: {
        cv: true,
    ...
    if (role === 'COMPANY' && analysis.job.companyId !== userId) {
      throw new UnauthorizedException('No tienes permiso para eliminar este candidato');
    }

    if (role === 'CANDIDATE' && analysis.cv.userId !== userId) {
      throw new UnauthorizedException('No tienes permiso para eliminar esta postulación');
    }

    // Delete associated interviews first to avoid foreign key constraints
