...
export class MailService {
  ...
  constructor() {
    this.transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.EMAIL_USER || 'iarecruiter026@gmail.com',
  ...
  async sendInterviewInvitation(candidateEmail: string, candidateName: string, jobTitle: string, date: string, location: string, notes: string) {
    const mailOptions = {
      from: `"IA Recruiter" <${process.env.EMAIL_USER || 'iarecruiter026@gmail.com'}>`,
      to: candidateEmail,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
          ...
          <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>📍 Lugar/Enlace:</strong> <a href="${location}">${location}</a></p>
