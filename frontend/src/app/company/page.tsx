...
export default function CompanyDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [jobsRes, statsRes, analysesRes, interviewsRes] = await Promise.all([
          api.get('/jobs'),
          api.get('/analysis/stats/company'),
          api.get('/analysis/company/all'),
          api.get('/interviews/company-interviews')
        ]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full bg-grid">
      ...
      {/* Entrevistas Programadas (Company) */}
      {interviews.length > 0 && (
        <div className="mt-12">
            ...
            {interviews.map((interview) => (
              <div key={interview.id} className="rounded-3xl border border-white/10 bg-slate-950/90 p-6 relative group">
                ...
                <div className="mb-4 pr-8">
                  ...
                  <p className="text-xs text-slate-400">Candidato: {interview.analysis.cv.user.name}</p>
                </div>
